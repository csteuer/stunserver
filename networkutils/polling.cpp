...
class CEpoll : public CBasicRefCount
...
uint32_t CEpoll::ToNativeFlags(uint32_t eventflags)
{
    ...
    if (eventflags & IPOLLING_READ)
        result |= EPOLLIN;
    if (eventflags & IPOLLING_WRITE)
        result |= EPOLLOUT;
    if (eventflags & IPOLLING_EDGETRIGGER)
        result |= EPOLLET;
#ifdef EPOLLRDHUP
    if (eventflags & IPOLLING_RDHUP)
        result |= EPOLLRDHUP;
#endif
    if (eventflags & IPOLLING_HUP)
        result |= EPOLLHUP;
    if (eventflags & IPOLLING_PRI)
        result |= EPOLLPRI;
    if (eventflags & IPOLLING_ERROR)
        result |= EPOLLERR;
...
uint32_t CEpoll::FromNativeFlags(uint32_t eventflags)
{
    ...
    if (eventflags & EPOLLIN)
        result |= IPOLLING_READ;
    if (eventflags & EPOLLOUT)
        result |= IPOLLING_WRITE;
    if (eventflags & EPOLLET)
        result |= IPOLLING_EDGETRIGGER;
#ifdef EPOLLRDHUP
    if (eventflags & EPOLLRDHUP)
        result |= IPOLLING_RDHUP;
#endif
...
class CPoll : public CBasicRefCount
...
uint32_t CPoll::FromNativeFlags(uint32_t eventflags)
{
    ...
    if (eventflags & POLLOUT)
        result |= IPOLLING_WRITE;
#ifdef POLLRDHUP
    if (eventflags & POLLRDHUP)
        result |= IPOLLING_RDHUP;
#endif
    if (eventflags & POLLHUP)
        result |= IPOLLING_HUP;
    if (eventflags & POLLPRI)
        result |= IPOLLING_PRI;
    if (eventflags & POLLERR)
        result |= IPOLLING_ERROR;
