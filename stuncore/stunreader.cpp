...
bool CStunMessageReader::IsFingerprintAttributeValid()
{
    ...
    computedValue = crc32(ptr, spBuffer->GetSize() - 8); // -8 because we're assuming the fingerprint attribute is 8 bytes and is the last attribute in the stream
                                                     ...
                                                     size_t keylength)
{
    ...
    ChkIf(_countAttributes == 0,
          E_FAIL); // if there's not attributes, there's definitely not a message
                   // integrity attribute
    ...
    // first, check to make sure that no other attributes (other than fingerprint)
    // follow the message integrity
    fNoOtherAttributesAfterIntegrity =
      (_indexMessageIntegrity == lastAttributeIndex) ||
      ((_indexMessageIntegrity == (lastAttributeIndex - 1)) &&
       (_indexFingerprint == lastAttributeIndex));
    ...
    len -= 4; // subtract the size of the message header (not including the
              // transaction id)

    // len should be the number of bytes from the start of the transaction ID up
...
HRESULT
CStunMessageReader::ValidateMessageIntegrityShort(const char* pszPassword)
{
    return ValidateMessageIntegrity((uint8_t*)pszPassword, strlen(pszPassword));
}

HRESULT CStunMessageReader::ValidateMessageIntegrityLong(
  const char* pszUser,
  const char* pszRealm,
  const char* pszPassword)
{
    ...
    uint8_t key[MAX_KEY_SIZE + 1]; // long enough for 64-char strings and two
                                   // semicolons and a null char
    uint8_t* pData = NULL;
    uint8_t* pDst = key;
    ...
    totallength =
      userLength + realmLength + passwordlength + 2; // +2 for two semi-colons
...
HRESULT CStunMessageReader::GetChangeRequest(
...
HRESULT CStunMessageReader::GetOtherAddress(CSocketAddress* pAddr)
{
    ...
    if (FAILED(hr))
    {
        // look for the legacy changed address attribute that a legacy (RFC 3489)
        // server would send
...
HRESULT CStunMessageReader::GetResponseOriginAddress(CSocketAddress* pAddr)
{
    ...
    if (FAILED(hr))
    {
        // look for the legacy address attribute that a legacy (RFC 3489) server
        // would send
                                                     ...
                                                     /*in-out*/ size_t size)
{
    ...
    // size needs to be 1 greater than attrib.size so we can properly copy over a
...
HRESULT CStunMessageReader::ReadHeader()
{
    ...
    // if we aren't in legacy format (the default), then the cookie field of the
    // transaction id must be the STUN_COOKIE
    fHeaderValid =
      (fHeaderValid && (_fAllowLegacyFormat || !_fMessageIsLegacyFormat));

    ChkIf(fHeaderValid == false, E_FAIL);

    _msgTypeNormalized = ((msgType & 0x000f) | ((msgType & 0x00e0) >> 1) |
                          ((msgType & 0x3E00) >> 2));
...
HRESULT CStunMessageReader::ReadBody()
{
    ...
    while (SUCCEEDED(hr) && (bytesConsumed < currentSize))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            // todo - if an attribute has no size, it's length is not padded by 4
            // bytes, right?
        ...
        if (SUCCEEDED(hr))
        {
            ...
            // if we have already read in more attributes than MAX_NUM_ATTRIBUTES,
            // then Insert call will fail (this is how we gate too many attributes)
        ...
        if (SUCCEEDED(hr))
        {
            bytesConsumed += sizeof(attributeType) + sizeof(attributeLength) +
                             attributeLength + paddingLength;
        }
    }

    // I don't think we could consume more bytes than stream size, but it's a
    // worthy check to still keep here
...
CStunMessageReader::ReaderParseState
CStunMessageReader::AddBytes(const uint8_t* pData, uint32_t size)
{
    HRESULT hr = S_OK;
    size_t currentSize;
    ...
    {
        // What?  After validating the body, the caller still passes in way too many
        // bytes?
        if (currentSize > (_msgLength + STUN_HEADER_SIZE))
        {
            _state = ParseError;
