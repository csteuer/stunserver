...
inline void _picohash_md5_init(void* ctx)
{
    _picohash_md5_ctx_t* md5_ctx = (_picohash_md5_ctx_t*)ctx;
...
inline void _picohash_md5_update(void* ctx, const void* data, size_t size)
{
    _picohash_md5_ctx_t* md5_ctx = (_picohash_md5_ctx_t*)ctx;
    ...
    if ((md5_ctx->lo = (saved_lo + size) & 0x1fffffff) < saved_lo)
        md5_ctx->hi++;
    md5_ctx->hi += (uint_fast32_t)(size >> 29);
    ...
    if (used)
    {
        ...
        size -= free;
        _picohash_md5_body(md5_ctx, md5_ctx->buffer, 64);
    }

    if (size >= 64)
    {
        data = _picohash_md5_body(md5_ctx, data, size & ~(unsigned long)0x3f);
        size &= 0x3f;
    }
...
inline void _picohash_md5_final(void* ctx, void* _digest)
{
    _picohash_md5_ctx_t* md5_ctx = (_picohash_md5_ctx_t*)ctx;
    ...
    digest[0] = md5_ctx->a;
    digest[1] = md5_ctx->a >> 8;
    digest[2] = md5_ctx->a >> 16;
    digest[3] = md5_ctx->a >> 24;
    digest[4] = md5_ctx->b;
    digest[5] = md5_ctx->b >> 8;
    digest[6] = md5_ctx->b >> 16;
    digest[7] = md5_ctx->b >> 24;
    digest[8] = md5_ctx->c;
    digest[9] = md5_ctx->c >> 8;
    digest[10] = md5_ctx->c >> 16;
    digest[11] = md5_ctx->c >> 24;
    digest[12] = md5_ctx->d;
    digest[13] = md5_ctx->d >> 8;
    digest[14] = md5_ctx->d >> 16;
    digest[15] = md5_ctx->d >> 24;
...
inline void _picohash_sha1_init(void* ctx)
{
    _picohash_sha1_ctx_t* s = (_picohash_sha1_ctx_t*)ctx;
...
inline void _picohash_sha1_update(void* ctx, const void* _data, size_t len)
{
    _picohash_sha1_ctx_t* s = (_picohash_sha1_ctx_t*)ctx;
...
inline void _picohash_sha1_final(void* ctx, void* digest)
{
    _picohash_sha1_ctx_t* s = (_picohash_sha1_ctx_t*)ctx;
...
inline void _picohash_sha256_init(void* c)
{
    _picohash_sha256_ctx_t* ctx = (_picohash_sha256_ctx_t*)c;
    ctx->curlen = 0;
    ctx->length = 0;
...
inline void _picohash_sha256_update(void* c, const void* data, size_t len)
{
    _picohash_sha256_ctx_t* ctx = (_picohash_sha256_ctx_t*)c;
...
inline void _picohash_sha256_final(void* c, void* digest)
{
    _picohash_sha256_ctx_t* ctx = (_picohash_sha256_ctx_t*)c;
    _picohash_sha256_do_final(ctx, digest, PICOHASH_SHA256_DIGEST_LENGTH);
}

inline void _picohash_sha224_init(void* c)
{
    _picohash_sha256_ctx_t* ctx = (_picohash_sha256_ctx_t*)c;
    ctx->curlen = 0;
    ctx->length = 0;
...
inline void _picohash_sha224_final(void* c, void* digest)
{
    _picohash_sha256_ctx_t* ctx = (_picohash_sha256_ctx_t*)c;
...
inline void picohash_init_md5(picohash_ctx_t* ctx)
{
    ...
    ctx->_reset = _picohash_md5_init;
    ctx->_update = _picohash_md5_update;
    ctx->_final = _picohash_md5_final;
...
inline void picohash_init_sha1(picohash_ctx_t* ctx)
{
    ...
    ctx->_reset = _picohash_sha1_init;
    ctx->_update = _picohash_sha1_update;
    ctx->_final = _picohash_sha1_final;
...
inline void picohash_init_sha224(picohash_ctx_t* ctx)
{
    ...
    ctx->_reset = _picohash_sha224_init;
    ctx->_update = _picohash_sha256_update;
    ctx->_final = _picohash_sha224_final;
...
inline void picohash_init_sha256(picohash_ctx_t* ctx)
{
    ...
    ctx->_reset = _picohash_sha256_init;
    ctx->_update = _picohash_sha256_update;
    ctx->_final = _picohash_sha256_final;
...
static void _picohash_hmac_final(void* c, void* digest)
{
    picohash_ctx_t* ctx = (picohash_ctx_t*)c;
...
static inline void _picohash_hmac_reset(void* c)
{
    picohash_ctx_t* ctx = (picohash_ctx_t*)c;
    ctx->_hmac.hash_reset(ctx);
...
inline void picohash_init_hmac(picohash_ctx_t* ctx, void (*initf)(picohash_ctx_t*), const void* key, size_t key_len)
{
    ...
    ctx->_reset = _picohash_hmac_reset;
    ctx->_final = _picohash_hmac_final;
